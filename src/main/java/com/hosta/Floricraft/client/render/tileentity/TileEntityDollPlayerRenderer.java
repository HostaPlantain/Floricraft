...
public class TileEntityDollPlayerRenderer extends TileEntitySpecialRenderer<TileEntityDollPlayer>{
	...
	@Override
	public void renderTileEntityAt(TileEntityDollPlayer te, double x, double y, double z, float partialTicks, int destroyStage)
	{
		renderDoll(te, x, y, z);
		renderItem(te, x, y, z);
	...
	private void renderDoll(TileEntityDollPlayer te, double x, double y, double z)
	{
        ...
        if (isSteve)
        {
            MODEL_STEVE.render(Minecraft.getMinecraft().thePlayer, 0.4F, 0.75F, 0.0F, 0.0F, 0.0F, 0.05F);
        }
        else
        {
            MODEL_ALEX.render(Minecraft.getMinecraft().thePlayer, 0.4F, 0.75F, 0.0F, 0.0F, 0.0F, 0.05F);
        }
