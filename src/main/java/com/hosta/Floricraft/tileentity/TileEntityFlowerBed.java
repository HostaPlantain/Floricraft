...
public class TileEntityFlowerBed extends TileEntityPlanter {
    ...
    {
        if((this.inventory[2] == null || this.inventory[2].isEmpty()) && stackIn != null && !stackIn.isEmpty())
        {
        ...
        else
        {
        	for (int i = 2; i >= 0; i--)
        	{
        		 if (this.inventory[i] != null && !this.inventory[i].isEmpty() && (stackIn == null || stackIn.isEmpty()))
        		 {
        			if (!this.world.isRemote)
        	        {
	...
	public void readFromNBT(NBTTagCompound nbt)
	{
        ...
        for(int i = 0; i < tags.tagCount(); i++)
        {
	        ...
	        if(slot >= 0 && slot < 3)
	        {
	        	this.setDisplayedItem(new ItemStack(tagCompound), i);
	        }
        }
	}
